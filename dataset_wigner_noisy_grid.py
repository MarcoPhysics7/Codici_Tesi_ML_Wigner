...
La Wigner è calcolata punto per punto su una griglia rumorosa.
...
OUTPUT_DATASET_DIR = "dataset_wigner_rumore_gaussianoo"
    ...
    for res in RESOLUTIONS:
        ...
        for coeff_path in tqdm(coeff_paths):
            try:
                ...
                W = np.zeros_like(X_noisy)
                for i in range(res):
                    for j in range(res):
                        x_ij = X_noisy[i, j]
                        p_ij = P_noisy[i, j]
                        val = wigner(rho, [x_ij], [p_ij])[0]
                        W[i, j] = val
